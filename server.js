...
app.post('/api/uat/sign', async (req, res) => {
  ...
  try {
    ...
    await request
      ...
      .query(`
        INSERT INTO UAT_Records
          (RequestId, EmployeeId, AssetId, SignedDate, SignatureUrl, PhotoUrl, VerifiedCondition, VerifiedLogin, Remarks)
        VALUES
          (@RequestId, @RecEmployeeId, @RecAssetId, GETDATE(), @SignatureUrl, @PhotoUrl, 1, 1, @Remarks)
      `);

    await transaction.commit();

    // 3) Fetch Asset Details for PDF
    ...
    doc.fontSize(12).text(`Custodian ID: ${employeeId}`);
...
// Needed for Azure Container Apps reachability
